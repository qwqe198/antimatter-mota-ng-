var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = 
{
	...
	if (core.hasItem('coin')) money *= core.itemCount("coin") + 1; // 幸运金币：双倍
	if (core.hasItem('I734')) money *= Math.pow((core.getFlag('world')), (1 + (core.itemCount('I734')))); // 世界等级
	...
	*/
	...
	if ((itemId.endsWith('Potion') || itemId == "poisonWine" || itemId == "weakWine" || itemId == "I572" || itemId == "curseWine" || itemId == "I619" || itemId == "I620" || itemId == "I621" || itemId == "I622") && core.material.items[itemId].cls == 'items')
		core.playSound('回血');
	else if ((itemId.endsWith('Gem') || itemId == "I576" || itemId == "I577" || itemId == "I578" || itemId == "I579" || itemId == "I584" || itemId == "I585" || itemId == "I586" || itemId == "I587" || itemId == "I635" || itemId == "I636" || itemId == "I637" || itemId == "I638") && core.material.items[itemId].cls == 'items')
		core.playSound('宝石')
	else
		core.playSound('获得道具');
	...
	return [
		...
		[4, "差的BP连击", "里程碑提升敌人每回合攻击次数。当前：11", "#ffee77"],
		[5, "好的BP连击", "里程碑大幅提升敌人每回合攻击次数。当前：144", "#ffee77"],
		...
		[22, "灵体", function (enemy) { return "敌人对角色造成" + (enemy.damage || 0) + "点固定伤害，该伤害可被攻防和减免，减免效果为攻防和的50倍。当前剩余：" + (enemy.damage - (hero.atk + hero.def) * 50 || 0) }, "#ff9977"],
		...
		[57, "血火囚牢", "无情的生机掠夺者。\n战斗前造成对方生命10%的真实伤害，并构建囚牢，封住对方17回合的行动。", "#DC143C"],
		[58, "长夜无光", "那个人终会迷失在妄自撕裂自身血肉的幻觉里。\n使对方陷入仿若真实的幻境，混乱30回合，并打上血之烙印，受到的伤害加倍。", "#2F4F4F"],
		[59, "匙弱", "这个敌人似乎对黄钥匙十分敏感。\n角色每拥有1把黄钥匙，该敌人伤害减少2%。", "#dfe650"],
		[60, "无力", "这个敌人非常惧怕强大的领悟技能。\n角色每升华一次初阶技能，该敌人生命减少5%。", "#89f562"],
		[61, "飓风", "这个敌人迅疾如风，引动了天地间的风元素异象。\n敌人首先发动一次20连击。", "#377d3d"],
		[62, "自爆", "强者在绝望之下最后的尊严。\n第400回合触发，立即死亡，对角色造成自身剩余生命*20的伤害。", "#597a80"],
		[63, "冰封万里", "天地茫茫，纯然一色，包容一切。\n战斗前冰封对手100回合，且每回合对对手造成自身攻击的8倍与防御的2倍之和的冰冻伤害。", "#6e43f0"],
		[64, "圣阵", "才德全尽谓之圣人，十圆无缺谓之圣阵。\n敌人布下圣阵，在战斗进行到第十、二十、五十回合时，分别对角色造成1.05次方、1.1次方、1.15次方角色与敌人攻防之和的穿透伤害。", "#d9964a"],
		[65, "执着", "铁杵磨成针。\n敌人每回合额外对角色造成角色生命百分之二的伤害。", "#cbb2d9"],
		[66, "和光同尘", "挫其锐，解其纷，和其光，同其尘。如春风般和煦，夏花般绚烂。\n战斗开始时以周围3*3方形内的事件数量为基准，每有1个事件，则每回合额外回复3000万点生命。", "#82e065"],
		[70, "生生不息", "生生不息，永恒不灭。\n每多一次维度提升，自身生命提升一倍，攻击、防御提升80%。", "#82e065"],
		[71, "锐利之锋", "斩断长空，势如破竹。\n每多一次维度提升，自身攻击提升一倍，生命、防御提升80%。", "#f5476a"],
		[72, "冷静之御", "沉静的守护者。\n每多一次维度提升，自身防御提升一倍，生命、攻击提升80%。", "#47b5f5"],
		[73, "百无禁忌", function (enemy) { return "百无禁忌，是为王道。\n每多一次维度提升，生命增加" + (enemy.value || 0) + "点，攻防增加" + (enemy.atkValue || 0) + "点。"; }, "#C0C0C0"],
		[74, "百无禁忌", function (enemy) { return "百无禁忌，是为王道。\n每多一次维度提升，生命增加" + (enemy.value || 0) + "点，攻击增加" + (enemy.atkValue || 0) + "点，防御增加" + (enemy.defValue || 0) + "点。"; }, "#C0C0C0"],
		[75, "命理升华", "由内而外成为更高层次生命。\n每多一次维度提升，自身三围属性提升一倍。", "#db4f80"],
		[76, "世界尽头", function (enemy) { return "这就是最后了！\n当你的维度提升达到" + (enemy.n || 0) + "时，该怪物将无法战胜。" }, "#2F4F4F"],
		[77, "不朽神剑", "唯一柄神剑在手，纵横无尽宇宙。\n每多一次维度提升，自身攻击提升1.1倍。", "#DAA520"],
		...
		[79, "双重反转",
			function (enemy) { return "战斗后，角色生命变为（里程碑*1e6-战后生命）当前：" + (11000000 + hero.hp - core.getDamage('loader', 6, 2, 'boss4_1') || 0) },
		],
		[80, "双重反转",
			function (enemy) { return "战斗后，角色生命变为（里程碑*1e6-战后生命）当前：" + (12000000 + hero.hp - core.getDamage('loader', 6, 2, 'boss4_1') || 0) },
		],
	...
	if (core.hasSpecial(mon_special, 70)) {
		mon_hp = Math.floor(mon_hp * Math.pow((2 * core.getFlag('un1')), (core.itemCount("I729"))));
		mon_atk = Math.floor(mon_atk * Math.pow((1.8 * core.getFlag('un1')), (core.itemCount("I729"))));
		mon_def = Math.floor(mon_def * Math.pow((1.8 * core.getFlag('un1')), (core.itemCount("I729"))));
	}
	// 攻击2
	if (core.hasSpecial(mon_special, 71)) {
		mon_hp = Math.floor(mon_hp * Math.pow((1.8 * core.getFlag('un1')), (core.itemCount("I729"))));
		mon_atk = Math.floor(mon_atk * Math.pow((2 * core.getFlag('un1')), (core.itemCount("I729"))));
		mon_def = Math.floor(mon_def * Math.pow((1.8 * core.getFlag('un1')), (core.itemCount("I729"))));
	}
	...
	if (core.hasSpecial(mon_special, 72)) {
		mon_hp = Math.floor(mon_hp * Math.pow((1.8 * core.getFlag('un1')), (core.itemCount("I729"))));
		mon_atk = Math.floor(mon_atk * Math.pow((1.8 * core.getFlag('un1')), (core.itemCount("I729"))));
		mon_def = Math.floor(mon_def * Math.pow((2 * core.getFlag('un1')), (core.itemCount("I729"))));
	}

	// 固定
	if (core.hasSpecial(mon_special, 73)) {
		mon_hp += (enemy.value || 0) * core.itemCount("I729");
		mon_atk += (enemy.atkValue || 0) * core.itemCount("I729");
		mon_def += (enemy.atkValue || 0) * core.itemCount("I729");
	}
	...
	if (core.hasSpecial(mon_special, 74)) {
		mon_hp += (enemy.value || 0) * core.itemCount("I729");
		mon_atk += (enemy.atkValue || 0) * core.itemCount("I729");
		mon_def += (enemy.defValue || 0) * core.itemCount("I729");
	}
	...
	// 攻击1.1
	if (core.hasSpecial(mon_special, 77)) {
		mon_atk = Math.floor(mon_atk * Math.pow((2.1 * core.getFlag('un1')), (core.itemCount("I729"))));
	}
	...
	if (core.hasSpecial(mon_special, 60)) {
		mon_hp *= 1 - ((core.getFlag('chujieshenghua') + 1) * 0.1);
	}
	...
	if (core.hasSpecial(mon_special, 39)) {
		mon_atk = mon_atk1 + hero_atk * 0.2;
		mon_def = mon_def1 + hero_def * 0.2;
	}
	// 饮剑
	...
	//升华
	hero_atk *= 1 + core.itemCount("I731");
	hero_def *= 1 + core.itemCount("I731");
	hero_mdef *= 1 + core.itemCount("I731");
	...
	// 尽头
	if (core.hasSpecial(mon_special, 76) && !core.itemCount("I729") >= enemy.n)
		return null; // 不可战斗
	...
	// 执着
	if (core.hasSpecial(mon_special, 65)) per_damage += hero_hp * 0.02;
	// 狂乱
	...
	// 2连击 & 3连击 & N连击
	if (core.hasSpecial(mon_special, 4)) per_damage *= 11;
	if (core.hasSpecial(mon_special, 5)) per_damage *= 144;
	...
	// 连击倍数
	if (core.hasItem("I629")) {
		hero_per_damage *= (1 + core.itemCount("I629"));
	}
	...
	if (core.hasSpecial(mon_special, 64)) {
		if (turn > 10)
			init_damage += Math.pow((mon_atk + hero_atk + mon_def + hero_def), 1.05);
		if (turn > 20)
			init_damage += Math.pow((mon_atk + hero_atk + mon_def + hero_def), 1.1);
		if (turn > 50) init_damage += Math.pow((mon_atk + hero_atk + mon_def + hero_def), 1.15);
	...
	if (core.hasSpecial(mon_special, 52)) {
		mon_hp -= 9 * hero_per_damage;
		if (turn >= 10) init_damage += mon_atk * 9;
	}
	...
	if (core.hasSpecial(mon_special, 22)) { // 灵体
		if (hero_mdef * 1 < enemy.damage || 0) {
			damage += enemy.damage || 0;
			damage -= (hero_atk + hero_def) * 50;
		} else damage = damage;
	}
	...
	if (core.hasSpecial(mon_special, 22)) { // 虚无
		if (hero_mdef * 5 < enemy.damage || 0) {
			damage += enemy.damage || 0;
			damage -= hero_mdef * 4;
		} else damage = damage;
	}
	...
	//减伤
	if (core.hasItem('I730')) {
		damage *= 1 - (core.itemCount("I730"));
	}
	...
	if (core.hasItem("I588")) {
		damage -= Math.ceil(hero_per_damage) * qing;
	...
	*/
	...
	if (!core.domStyle.isVertical) {
		// 横屏模式
		core.drawLine(ctx, 0, 28, 128, 28, "#FFFFFF", 1);
		...
		core.drawLine(ctx, 0, 122, 128, 122, "#FFFFFF", 1);
		core.drawLine(ctx, 0, 147, 128, 147, "#FFFFFF", 1);
		core.drawLine(ctx, 0, 172, 128, 172, "#FFFFFF", 1);
		core.drawLine(ctx, 0, 197, 128, 197, "#FFFFFF", 1);
		core.drawLine(ctx, 0, 222, 128, 222, "#FFFFFF", 1);
		core.drawLine(ctx, 23, 222, 23, 299, "#FFFFFF", 1);
		core.drawLine(ctx, 0, 299, 128, 299, "#FFFFFF", 1);

		if (core.hasItem('I474')) { core.drawImage(ctx, 'E.png', 29, 368, 60, 60); } else if (core.hasItem('I471')) { core.drawImage(ctx, 'E.png', 29, 368, 60, 60); } else if (core.hasItem('I475')) { core.drawImage(ctx, 'H.png', 29, 368, 60, 60); } else { core.drawImage(ctx, 'C.png', 31, 368, 60, 60); }
		...
		core.drawImage(ctx, "t.png", 79, 280, 16, 16);
		_fillBoldTextWithFontCheck(core.itemCount("jumpShoes"), 103, 293, '#c64fdb');
		core.drawImage(ctx, "lv.png", 17, 378, 16, 16);
		_fillBoldTextWithFontCheck(core.itemCount("greenKey"), 22, 410, 'LightGreen');
		core.drawImage(ctx, "w.png", 52, 378, 16, 16);
		_fillBoldTextWithFontCheck(core.itemCount("I729"), 58, 410, '#FFDEAD');
		_fillBoldTextWithFontCheck(('正物质'), 98, 390, '#778899');
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('exp')), 98, 410, "#A9A9A9");
		...
		_fillBoldTextWithFontCheck(('反物质'), 86, 48, '#98FB98');
		_fillBoldTextWithFontCheck(('累计了'), 54, 71, '#E0FFFF');
		_fillBoldTextWithFontCheck(core.itemCount('I848'), 34, 94, '#FFCCAA');
		_fillBoldTextWithFontCheck(('星系'), 86, 94, '#E0FFFF');

		core.drawImage(ctx, "w.png", 140, 59, 16, 16);
		_fillBoldTextWithFontCheck(core.itemCount("I729"), 148, 96, '#FFDEAD');
		_fillBoldTextWithFontCheck(('正物质'), 148, 25, '#778899');
		_fillBoldTextWithFontCheck(core.formatBigNumber(core.getRealStatus('exp')), 148, 48, "#A9A9A9");
	...
	return [
		...
		'redPotion', 'bluePotion', 'greenPotion', 'yellowPotion', 'superPotion', 'I619', 'I620', 'I621', 'I622',
